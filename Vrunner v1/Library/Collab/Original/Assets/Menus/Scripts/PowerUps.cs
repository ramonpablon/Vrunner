...
public class PowerUps : MonoBehaviour {
    ...
    void Update () {
		...
		else if (Input.GetKey(KeyCode.F))// ativa freefall
		{
			GameModeBH.modo =2;
			GameModeBH.emTransicao =true;
		}
        else
            powerups = 0;
    }
    ...
    public void ChangePowerUp(Image temp)
    {
        ...
        {
			//entao ponho a duração do freefall aqui?
	        print("free");

		}
        if (temp.sprite == freeLinf)
        {
			//a deixa ele brilhando kkk tipo o mario quando pega a estrela
            print("freelinf");
			//mas ai tipo ele da o dash, quebra as coisas sem morrer, mas eh um dash tipo no temple run q o cara corre um tempao ou soh uma coisinha curta pra fugir do olho?
			//soh vo descomentar la do freefall ai ja upo
        }
